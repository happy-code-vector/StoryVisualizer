...
function initializeDatabase() {
  ...
  try {
    const modelStmt = db.prepare(`
      INSERT OR IGNORE INTO models (name, type, link, is_default)
      VALUES (?, ?, ?, ?)
    `)
    modelStmt.run('nano-banana', 'character', 'https://fal.run/fal-ai/nano-banana/', 1)
    modelStmt.run('flux-dev', 'scene', 'https://fal.run/fal-ai/flux/dev', 1)
  } catch (error) {
    console.error('Error inserting default models:', error)
  }
}
...
export function getModelsByType(type: 'character' | 'scene'): ModelRecord[] {
  const stmt = db.prepare(`
    SELECT id, name, type, link, is_default as isDefault, created_at as createdAt
    FROM models
    WHERE type = ?
    ORDER BY is_default DESC, name
  `)
  
  return stmt.all(type) as ModelRecord[]
}

// Get model by name
export function getModelByName(name: string): ModelRecord | null {
  const stmt = db.prepare(`
    SELECT id, name, type, link, is_default as isDefault, created_at as createdAt
    FROM models
    WHERE name = ?
  ...
  return stmt.get(name) as ModelRecord || null
}

// Add a new model
export function addModel(name: string, type: 'character' | 'scene', link: string, isDefault: boolean = false): ModelRecord | null {
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO models (name, type, link, is_default)
      VALUES (?, ?, ?, ?)
    `)
    
    const result = stmt.run(name, type, link, isDefault ? 1 : 0)
...
export function deleteModelById(id: number): boolean {
  try {
    const stmt = db.prepare(`
      DELETE FROM models
      WHERE id = ?
...
// Re-export Supabase service functions to maintain compatibility
export {
  saveStoryAnalysis,
  getAllStories,
  getStoryById,
