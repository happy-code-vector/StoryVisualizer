...
export async function POST(request: Request) {
  try {
    ...
    // Check if trying to disable a root user
    ...
    if (userData.role === 'root') {
      return NextResponse.json({ error: 'Cannot disable root users' }, { status: 403 })
    }

    // Disable user by setting verified to false
    ...
    return NextResponse.json({ error: error.message || 'Failed to disable user' }, { status: 500 })
  }
}